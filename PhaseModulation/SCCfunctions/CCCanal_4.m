function [SACSCCfunctions,SACSCCmetrics,paramsOUT] = CCCanal_4(SpikeTrains,paramsIN,PLOT_15panel)
% File: CCCanal.m
...
% CCCanal_4: quick HACK: 1 basic RUN using all spike REPs - with updates
% from CCCanal_3 BOOTSTRAP 
%
...
if ~isfield(paramsOUT,'CCCenv_TOUSE')
	paramsOUT.CCCenv_TOUSE='10-300, subBIAS';
end
...
if ~isfield(paramsOUT,'DELAYbinwidth_sec')
...
paramsOUT.BOOTSTRAP_percentdata=1.00;  % Percent of AN reps to include in each RUN
...
% % 	end
% % end
paramsOUT.BOOTSTRAP_Navgs=1;
for i=1:2
	for j=1:2
		BOOTinds{i,j}{1}=1:length(SpikeTrains{i,j});
	end
end
...
% 			disp(sprintf('   ... Computing SACs/SCCs for RANDOM spikes (for bias removal) ...'))
	...
	% default list
	default_PSD_LHfreqs_Hz=[0 NYQ_Hz; 0 CF_Hz; 0 300; 10 300; 10 CF_Hz; 0 50; 0 100];
...
%% HACK to make get rid of BOOTSTRAPPING
SACSCCfunctions=SACSCCfunctions{1};
SACSCCmetrics=SACSCCmetrics{1};
