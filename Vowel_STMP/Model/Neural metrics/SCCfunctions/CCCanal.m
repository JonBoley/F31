...
if ~LOADexistingDATA
	...
	if ~isfield(paramsOUT,'SCC_onsetIGNORE_sec')
		paramsOUT.SCC_onsetIGNORE_sec=0.05;  %starting time to ignore
	end
	...
	DELAYbinwidth_sec=50e-6;          paramsOUT.DELAYbinwidth_sec=DELAYbinwidth_sec;
	% FFT window length - to get 1-Hz sampling
	Nfft_psd=round(1/DELAYbinwidth_sec);  % 20000
	...
	paramsOUT.MAXspikes=2000;  %5000% If used, windowSTs will cut out extra REPs beyond MAXspikes - used to avoid memory limits
	...
	disp('   ... Computing SACs/SCCs for RANDOM spikes (for bias removal) ...')
...
%  - set up to run only COL 1 for [CFi, CFi]
% 7) clean up code for public use
...
SACSCCmetrics.CDscc=NaN;
SACSCCmetrics.CDenv=NaN;
SACSCCmetrics.CDtfs=NaN;
