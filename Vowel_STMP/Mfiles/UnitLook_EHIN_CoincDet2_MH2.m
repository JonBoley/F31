...
% updated: Jun 12, 2009 - for VowelSTMP directory
%
...
% 	loadBOOL=1;
	...
	if loadBOOL
        ...
        % in case these were overwritten...
        data_dir=fullfile(ROOT_dir,'ExpData',ExpName);
        unitdata_dir=fullfile(data_dir,'UNITSdata');
...
if ~loadBOOL
	...
	if doSCC
		...
		SCC_octOFFSET2=0.05; % parameter of the offset between higher BF and Nominal BF
		...
		paramsIN.SCC.EndTime_sec=2^-SCC_octOFFSET2*.400;
...
% 								NSCCs{ROWind,ATTind}{SCCind}=NSCCs{ROWind,ATTind}{SCCind}* ...
% 									NSCCbinwidth_sec * NSCC_avgrates{ROWind,ATTind}{SCCind}{1} * ...
% 									NSCC_avgrates{ROWind,ATTind}{SCCind}{2};
								...
								F0per_us=1/yTEMP.FeatureFreqs_Hz{1}(1)*1e6;
								disp(sprintf('Feature: %s; SNR: %.f dB  --  CAN''T compute SCC # %d between BFs %d and %d (MISSING DATA) ........', ...
									FeaturesText{FeatIND},Nattens_dB(ATTind)+dBAtt_2_SNR,SCCind,NSCC_BFinds{SCCind}))
...
if doSCC
	...
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	XLIMITS_scc=1.5*[-PERhist_XMAX PERhist_XMAX];
	YLIMITS_scc=[0 8.2];
					...
					for SCCind=1:length(NSCCs{ROWind,ATTind})
						...
						if ~isempty(NSCCs{ROWind,ATTind}{SCCind})
							plot(NSCC_delays_usec{ROWind,ATTind}{SCCind}/1000, ...
								NSCCs{ROWind,ATTind}{SCCind},'LineWidth',LINEwidth)
							hold on
							plot(zeros(1,2),YLIMITS_scc,'k:','LineWidth',LINEwidth/2)
							% Plot values at 0 delay and CD
							plot(0,NSCC_0delay{ROWind,ATTind}{SCCind},'sk')
							plot(NSCC_CDs_usec{ROWind,ATTind}{SCCind}/1000,NSCC_peaks{ROWind,ATTind}{SCCind},'or')
						end
					end
					xlabel('Delay (ms)')
					ylabel('[CoincDet] Rate (sp/sec)')
					if ROWind==1
						title(sprintf('     Exp%s, Unit %s: BF=%.2f kHz, Thr=%.f dB SPL, SR=%.1f sps, Q10=%.1f\n%s @ %.f dB SPL, SNR = %.f dB,  Harm: %d, Polarity: %d  [SCC: BF+%.2f oct. re BF+%.2f oct]', ...
							ExpDate,UnitName,unit.Info.BF_kHz,unit.Info.Threshold_dBSPL,unit.Info.SR_sps,unit.Info.Q10,FeaturesText{FeatIND}, ...
							yTEMP.levels_dBSPL,yTEMP.Nattens_dB(ATTind)+dBAtt_2_SNR,HarmonicsIND,PolarityIND,SCC_octOFFSET2,SCC_octOFFSET1),'units','norm','pos',[.1 1 0],'HorizontalAlignment','left')
					else
						title(sprintf('%s @ %.f dB SPL, SNR = %.f dB,  Harm: %d, Polarity: %d  [SCC: BF+%.2f oct. re BF+%.2f oct]',FeaturesText{FeatIND}, ...
							yTEMP.levels_dBSPL,yTEMP.Nattens_dB(ATTind)+dBAtt_2_SNR,HarmonicsIND,PolarityIND,SCC_octOFFSET2,SCC_octOFFSET1),'units','norm','pos',[.1 1 0],'HorizontalAlignment','left')
					end
					xlim(XLIMITS_scc)
					PLOThand=eval(['h' num2str(PLOTnum)]);
		...
		for FeatIND=FeatINDs
			...
			if ~isempty(yTEMP)
				...
				for SCCind=1:length(NSCCs{ROWind,ATTind})
					...
					if ~isempty(NSCCs{ROWind,ATTind}{SCCind})
						plot(NSCC_delays_usec{ROWind,ATTind}{SCCind}/1000, ...
							NSCCs{ROWind,ATTind}{SCCind},'LineWidth',LINEwidth,'Color',ATTENcolors{ATTind})
						hold on
						if SCCind==1
							LEGtext{length(LEGtext)+1}=sprintf('%.f dB',Nattens_dB(ATTind)+dBAtt_2_SNR);
						end
					...
					for ATTind2=fliplr(find(Nattens_dB~=max(Nattens_dB)))
						if ~isempty(NSCCs{ROWind,ATTind2}{SCCind})
							plot(NSCC_delays_usec{ROWind,ATTind2}{SCCind}/1000, ...
								NSCCs{ROWind,ATTind2}{SCCind},'LineWidth',LINEwidth,'Color',ATTENcolors{ATTind2})
							hold on
							if SCCind==1
								LEGtext{length(LEGtext)+1}=sprintf('%.f dB',Nattens_dB(ATTind2)+dBAtt_2_SNR);
							end
...
% 						semilogy(zeros(1,2),YLIMITS,'k:','LineWidth',LINEwidth/2)
				...
				if ROWind==1
					title(sprintf('     Exp%s, Unit %s: BF=%.2f kHz, Thr=%.f dB SPL, SR=%.1f sps, Q10=%.1f\n%s @ %.f dB SPL,  Harm: %d, Polarity: %d  [SCC: BF+%.2f oct. re BF+%.2f oct]', ...
						ExpDate,UnitName,unit.Info.BF_kHz,unit.Info.Threshold_dBSPL,unit.Info.SR_sps,unit.Info.Q10,FeaturesText{FeatIND}, ...
						yTEMP.levels_dBSPL,HarmonicsIND,PolarityIND,SCC_octOFFSET2,SCC_octOFFSET1),'units','norm','pos',[.1 1 0],'HorizontalAlignment','left')
				else
					title(sprintf('%s @ %.f dB SPL,  Harm: %d, Polarity: %d  [SCC: BF+%.2f oct. re BF+%.2f oct]',FeaturesText{FeatIND}, ...
						yTEMP.levels_dBSPL,HarmonicsIND,PolarityIND,SCC_octOFFSET2,SCC_octOFFSET1),'units','norm','pos',[.1 1 0],'HorizontalAlignment','left')
				end
				xlim(XLIMITS_scc)
				PLOThand=eval(['h' num2str(PLOTnum)]);
