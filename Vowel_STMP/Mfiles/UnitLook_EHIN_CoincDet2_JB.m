function [UnitCF,UnitThresh,UnitQ10,...
    Rate_failpoint,Rate_fail_limit,...
    ALSR_failpoint,ALSR_fail_limit,...
...
if ~loadBOOL
    ...
    if isfield(unit,'EHvN_reBF_simFF') || isfield(unit,'EHvLTASS_reBF_simFF')
        ...
        for FeatIND=FeatINDs
            try
                if ~isempty(unit.EHvLTASS_reBF_simFF.F1{HarmonicsIND,PolarityIND})
                            ...
                            if ~emptySCC
                                ...
                                try
                                    NSCCs_sps{ROWind,ATTind}{SCCind}=NSCCs{ROWind,ATTind}{SCCind}* ...
                                        ...
                                        cell2mat(unit.EHvLTASS_reBF_simFF.F1{HarmonicsIND,PolarityIND}.TimeFact(1,NSCC_BFinds{SCCind}(1))) * ...
                                        NSCC_avgrates{ROWind,ATTind}{SCCind}{2} * ...
                                        cell2mat(unit.EHvLTASS_reBF_simFF.F1{HarmonicsIND,PolarityIND}.TimeFact(1,NSCC_BFinds{SCCind}(2)));
                                catch
                                    NSCCs_sps{ROWind,ATTind}{SCCind}=NSCCs{ROWind,ATTind}{SCCind}* ...
                                        NSCCbinwidth_sec * ...
                                        NSCC_avgrates{ROWind,ATTind}{SCCind}{1} * ...
                        ...
                        if ~isempty(PERhistXs_sec{ROWind,ATTind2}{BFind})
                            ...
                            if ismember(BFind,find(abs(log2(BFs_kHz{ROWind,ATTind}/unit.Info.BF_kHz))<BFoctCRIT))
                                ...
                                if ATTind2==yTEMP.EqualSL_index
                                    LEGtext{length(LEGtext)+1}=sprintf('%.f dB SNR',yTEMP.SNR_EqualSL);%+dBAtt_2_SNR);
                                else
                                    LEGtext{length(LEGtext)+1}='0 dB SNR';%sprintf('%.f dB',Nattens_dB(ATTind2)+dBAtt_2_SNR);
                                end
                            end
                        end
                    end
...
figure(502);clf
set(gcf,'Name','Synchronized Correlations');
NUMcols=1;
NUMrows=length(FeatINDs);
ROWind=0;
    ...
    for FeatIND=FeatINDs
        ...
        if ~isempty(yTEMP)
            ...
            for BFind=1:length(BFs_kHz{ROWind,ATTind})
                ...
                %%%% Rho*Synch Plot
                PLOTnum=ROWind;%(ROWind-1)*NUMcols+3;
                eval(['h' num2str(PLOTnum) '=subplot(NUMrows,NUMcols,PLOTnum);'])
                semilogy(data2plot{ROWind,ATTind}(:,4)'.*Synchs{ROWind,ATTind},BFs_kHz{ROWind,ATTind},'*-','Color',ATTENcolors{ATTind})
                hold on
                for ATTind2=fliplr(find(Nattens_dB~=max(Nattens_dB)))
                    semilogy(data2plot{ROWind,ATTind}(:,4)'.*Synchs{ROWind,ATTind2},BFs_kHz{ROWind,ATTind2},'*-','Color',ATTENcolors{ATTind2})
                end
                semilogy([-1000 1000],unit.Info.BF_kHz*[1 1],'k:')
                xlabel(sprintf('Rho * Synch Coef (to F0)'));%%s)',FeaturesText{FeatIND}))
                PLOThand=eval(['h' num2str(PLOTnum)]);
                xlim(XLIMITS_synch)
                set(PLOThand,'XDir','reverse')
                set(PLOThand,'YTick',YTICKS,'YTickLabel',YTICKS)
