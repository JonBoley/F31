function UnitLook_EH_CoincDet2(UnitName)
...
if doSCC
    ...
    SMP_NSCC_Oct_at_Rho=cell(length(FeatINDs),length(levels_dBSPL));
    SMP_NSCC_CD_at_Oct=cell(length(FeatINDs),length(levels_dBSPL));
    for ATTind=1:length(levels_dBSPL)
        ROWind=0;
        for FeatIND=FeatINDs
            ROWind=ROWind+1;
...
if doSCC
	...
	for ATTind=1:length(levels_dBSPL)
		...
		if ~isnan(SensitivitySlopes_nscc0{1}(ATTind))
			...
			plot(xdata,ydata,'Marker','none','Color',ATTENcolors{ATTind},'LineStyle','-')
			hold on
			LEGtext{length(LEGtext)+1}=sprintf('%.f dB (slope=%.3f)',levels_dBSPL(ATTind),SensitivitySlopes_nscc0{1}(ATTind));  % sp/sec/dB
		end
	end
	for ATTind=1:length(levels_dBSPL)
		for FeatIND=FeatINDs
			if ~isnan(SMP_NSCC_0delay{1,ATTind}(FeatIND))
				plot(yTEMP.levels_dBSPL(ATTind)+FeatureLevels_dB(FeatIND),SMP_NSCC_Oct_at_Rho{FeatIND-2,ATTind},'Marker',FEATmarkers{FeatIND},'Color',ATTENcolors{ATTind},'LineStyle','none')
				hold on
			end
		end
	end
...
% 	set(gca,'FontSize',FIG.FontSize)
	...
	for ATTind=1:length(levels_dBSPL)
		...
		if ~isnan(SensitivitySlopes_nsccCD{1}(ATTind))
			...
			hold on
			LEGtext{length(LEGtext)+1}=sprintf('%.f dB SPL (%.2f usec/dB)',levels_dBSPL(ATTind),SensitivitySlopes_nsccCD{1}(ATTind));
		end
	end
	for ATTind=1:length(levels_dBSPL)
		for FeatIND=FeatINDs
			if ~isnan(SMP_NSCC_CD{1,ATTind}(FeatIND))
				plot(yTEMP.levels_dBSPL(ATTind)+FeatureLevels_dB(FeatIND),SMP_NSCC_CD_at_Oct{FeatIND-2,ATTind}/1000, ...
...
hold off
% ylim([ymin ymax])
ylim([yminFACT ymaxFACT]*SensitivitySlopes_rate(1))
title(sprintf('     Exp%s, Unit %s: BF=%.2f kHz, Thr=%.f dB SPL, SR=%.1f sps, Q10=%.1f\nRATE', ...
	ExpDate,UnitName,unit.Info.BF_kHz,unit.Info.Threshold_dBSPL,unit.Info.SR_sps,unit.Info.Q10), ...
	'units','norm')
% title('RATE')
