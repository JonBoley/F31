function [SACSCCfunctions,SACSCCmetrics,paramsOUT] = CCCanal_2(SpikeTrains,paramsIN,PLOT_15panel)
% File: CCCanal.m
...
% Number of REPS for Averaging CCCs
paramsOUT.Nreps_CCCavging=5;
...
for CCCrep=1:paramsOUT.Nreps_CCCavging

	disp(sprintf('   ... Computing SACs/SCCs for AN spikes: REP %d of %d ...',CCCrep,paramsOUT.Nreps_CCCavging))
	...
	SACSCCmetrics{CCCrep}.sums.sumPSDsc_A_CCCenv = sum(SACSCCs.PSDsc_A(CCCenv_LOWmod_ind:CCCenv_HIGHmod_ind));
	if ~isnan(NumDrivenSpikes(2,1))
		SACSCCmetrics{CCCrep}.sums.sumPSDsc_B_CCCenv = sum(SACSCCs.PSDsc_B(CCCenv_LOWmod_ind:CCCenv_HIGHmod_ind));
		SACSCCmetrics{CCCrep}.sums.sumCSDsc_AB_CCCenv = sum(SACSCCs.CSDsc_AB(CCCenv_LOWmod_ind:CCCenv_HIGHmod_ind));
		SACSCCmetrics{CCCrep}.sums.sumPSDsc_Arand_CCCenv = sum(SACSCCs_rand.PSDsc_A(CCCenv_LOWmod_ind:CCCenv_HIGHmod_ind));
		SACSCCmetrics{CCCrep}.sums.sumPSDsc_Brand_CCCenv = sum(SACSCCs_rand.PSDsc_B(CCCenv_LOWmod_ind:CCCenv_HIGHmod_ind));
		SACSCCmetrics{CCCrep}.sums.sumCSDsc_ABrand_CCCenv = sum(SACSCCs_rand.CSDsc_AB(CCCenv_LOWmod_ind:CCCenv_HIGHmod_ind));
	end

	% 0-CCCenv_HIGH based frequency range (0-300 Hz: for SCpeak3)
	SACSCCmetrics{CCCrep}.sums.sumPSDsc_A_0_CCCenv = sum(SACSCCs.PSDsc_A(CCCenv_ZEROmod_ind:CCCenv_HIGHmod_ind));
	if ~isnan(NumDrivenSpikes(2,1))
		SACSCCmetrics{CCCrep}.sums.sumPSDsc_B_0_CCCenv = sum(SACSCCs.PSDsc_B(CCCenv_ZEROmod_ind:CCCenv_HIGHmod_ind));
		SACSCCmetrics{CCCrep}.sums.sumCSDsc_AB_0_CCCenv = sum(SACSCCs.CSDsc_AB(CCCenv_ZEROmod_ind:CCCenv_HIGHmod_ind));
		SACSCCmetrics{CCCrep}.sums.sumPSDsc_Arand_0_CCCenv = sum(SACSCCs_rand.PSDsc_A(CCCenv_ZEROmod_ind:CCCenv_HIGHmod_ind));
		SACSCCmetrics{CCCrep}.sums.sumPSDsc_Brand_0_CCCenv = sum(SACSCCs_rand.PSDsc_B(CCCenv_ZEROmod_ind:CCCenv_HIGHmod_ind));
		SACSCCmetrics{CCCrep}.sums.sumCSDsc_ABrand_0_CCCenv = sum(SACSCCs_rand.CSDsc_AB(CCCenv_ZEROmod_ind:CCCenv_HIGHmod_ind));
	end

	% 0-CF frequency range
	SACSCCmetrics{CCCrep}.sums.sumPSDsc_A_0_CF = sum(SACSCCs.PSDsc_A(CCCenv_ZEROmod_ind:CCCenv_CFmod_ind));
	if ~isnan(NumDrivenSpikes(2,1))
		SACSCCmetrics{CCCrep}.sums.sumPSDsc_B_0_CF = sum(SACSCCs.PSDsc_B(CCCenv_ZEROmod_ind:CCCenv_CFmod_ind));
		SACSCCmetrics{CCCrep}.sums.sumCSDsc_AB_0_CF = sum(SACSCCs.CSDsc_AB(CCCenv_ZEROmod_ind:CCCenv_CFmod_ind));
		SACSCCmetrics{CCCrep}.sums.sumPSDsc_Arand_0_CF = sum(SACSCCs_rand.PSDsc_A(CCCenv_ZEROmod_ind:CCCenv_CFmod_ind));
		SACSCCmetrics{CCCrep}.sums.sumPSDsc_Brand_0_CF = sum(SACSCCs_rand.PSDsc_B(CCCenv_ZEROmod_ind:CCCenv_CFmod_ind));
		SACSCCmetrics{CCCrep}.sums.sumCSDsc_ABrand_0_CF = sum(SACSCCs_rand.CSDsc_AB(CCCenv_ZEROmod_ind:CCCenv_CFmod_ind));
	end

	% CCCenvLOW-CF frequency range (10-CF: for CCCenv4)
	SACSCCmetrics{CCCrep}.sums.sumPSDsc_A_CCCenv_CF = sum(SACSCCs.PSDsc_A(CCCenv_LOWmod_ind:CCCenv_CFmod_ind));
	if ~isnan(NumDrivenSpikes(2,1))
		SACSCCmetrics{CCCrep}.sums.sumPSDsc_B_CCCenv_CF = sum(SACSCCs.PSDsc_B(CCCenv_LOWmod_ind:CCCenv_CFmod_ind));
		SACSCCmetrics{CCCrep}.sums.sumCSDsc_AB_CCCenv_CF = sum(SACSCCs.CSDsc_AB(CCCenv_LOWmod_ind:CCCenv_CFmod_ind));
		SACSCCmetrics{CCCrep}.sums.sumPSDsc_Arand_CCCenv_CF = sum(SACSCCs_rand.PSDsc_A(CCCenv_LOWmod_ind:CCCenv_CFmod_ind));
		SACSCCmetrics{CCCrep}.sums.sumPSDsc_Brand_CCCenv_CF = sum(SACSCCs_rand.PSDsc_B(CCCenv_LOWmod_ind:CCCenv_CFmod_ind));
		SACSCCmetrics{CCCrep}.sums.sumCSDsc_ABrand_CCCenv_CF = sum(SACSCCs_rand.CSDsc_AB(CCCenv_LOWmod_ind:CCCenv_CFmod_ind));
	end

	% 0-Fs/2 frequency range
	SACSCCmetrics{CCCrep}.sums.sumPSDsc_A_0_NYQ = sum(SACSCCs.PSDsc_A(CCCenv_ZEROmod_ind:CCCenv_NYQmod_ind));
	if ~isnan(NumDrivenSpikes(2,1))
		SACSCCmetrics{CCCrep}.sums.sumPSDsc_B_0_NYQ = sum(SACSCCs.PSDsc_B(CCCenv_ZEROmod_ind:CCCenv_NYQmod_ind));
		SACSCCmetrics{CCCrep}.sums.sumCSDsc_AB_0_NYQ = sum(SACSCCs.CSDsc_AB(CCCenv_ZEROmod_ind:CCCenv_NYQmod_ind));
		SACSCCmetrics{CCCrep}.sums.sumPSDsc_Arand_0_NYQ = sum(SACSCCs_rand.PSDsc_A(CCCenv_ZEROmod_ind:CCCenv_NYQmod_ind));
		SACSCCmetrics{CCCrep}.sums.sumPSDsc_Brand_0_NYQ = sum(SACSCCs_rand.PSDsc_B(CCCenv_ZEROmod_ind:CCCenv_NYQmod_ind));
		SACSCCmetrics{CCCrep}.sums.sumCSDsc_ABrand_0_NYQ = sum(SACSCCs_rand.CSDsc_AB(CCCenv_ZEROmod_ind:CCCenv_NYQmod_ind));
	end

	%% Compute metrics
	%% Peak Heights
	% SAC peaks - this is CI from Joris et al 2006 (HR) [don't subtract 1]
	SACSCCmetrics{CCCrep}.SACpeak_A=max(SACSCCs.SAC_A_avg);
	if ~isnan(NumDrivenSpikes(2,1))
		SACSCCmetrics{CCCrep}.SACpeak_B=max(SACSCCs.SAC_B_avg);
		SACSCCmetrics{CCCrep}.SCCpeak_AB=max(SACSCCs.SCC_AB_avg);
	end
	% DIFCOR peaks
	SACSCCmetrics{CCCrep}.DCpeak_A=max(SACSCCs.DIFCOR_A);
	if ~isnan(NumDrivenSpikes(2,1))
		SACSCCmetrics{CCCrep}.DCpeak_B=max(SACSCCs.DIFCOR_B);
		SACSCCmetrics{CCCrep}.DCpeak_AB=max(SACSCCs.DIFCOR_AB);
	end
	% SUMCOR peaks - don't subtract 1 (Louage et al 2004)
	% SCpeak2: raw peaks
	SACSCCmetrics{CCCrep}.SCpeak2_A=max(SACSCCs.SUMCOR_A);
	if ~isnan(NumDrivenSpikes(2,1))
		SACSCCmetrics{CCCrep}.SCpeak2_B=max(SACSCCs.SUMCOR_B);
		SACSCCmetrics{CCCrep}.SCpeak2_AB=max(SACSCCs.SUMCOR_AB);
	end
	%% USE THIS ONE: SCpeak: Eq. 2 = adjusted SC peak height [0,CF]/[0,Fs/2]
	SACSCCmetrics{CCCrep}.SCpeak_A=1+(SACSCCmetrics{CCCrep}.SCpeak2_A-1)*SACSCCmetrics{CCCrep}.sums.sumPSDsc_A_0_CF/SACSCCmetrics{CCCrep}.sums.sumPSDsc_A_0_NYQ;
	if ~isnan(NumDrivenSpikes(2,1))
		SACSCCmetrics{CCCrep}.SCpeak_B=1+(SACSCCmetrics{CCCrep}.SCpeak2_B-1)*SACSCCmetrics{CCCrep}.sums.sumPSDsc_B_0_CF/SACSCCmetrics{CCCrep}.sums.sumPSDsc_B_0_NYQ;
		SACSCCmetrics{CCCrep}.SCpeak_AB=1+(SACSCCmetrics{CCCrep}.SCpeak2_AB-1)*SACSCCmetrics{CCCrep}.sums.sumCSDsc_AB_0_CF/SACSCCmetrics{CCCrep}.sums.sumCSDsc_AB_0_NYQ;
	end
	% SCpeak_3: alternative adjusted SC peak height [0,300]/[0,Fs/2]
	SACSCCmetrics{CCCrep}.SCpeak3_A=1+(SACSCCmetrics{CCCrep}.SCpeak2_A-1)*SACSCCmetrics{CCCrep}.sums.sumPSDsc_A_0_CCCenv/SACSCCmetrics{CCCrep}.sums.sumPSDsc_A_0_NYQ;
	if ~isnan(NumDrivenSpikes(2,1))
		SACSCCmetrics{CCCrep}.SCpeak3_B=1+(SACSCCmetrics{CCCrep}.SCpeak2_B-1)*SACSCCmetrics{CCCrep}.sums.sumPSDsc_B_0_CCCenv/SACSCCmetrics{CCCrep}.sums.sumPSDsc_B_0_NYQ;
		SACSCCmetrics{CCCrep}.SCpeak3_AB=1+(SACSCCmetrics{CCCrep}.SCpeak2_AB-1)*SACSCCmetrics{CCCrep}.sums.sumCSDsc_AB_0_CCCenv/SACSCCmetrics{CCCrep}.sums.sumCSDsc_AB_0_NYQ;
	end

	if ~isnan(NumDrivenSpikes(2,1))

		%% Neural Cross Correlation Coefficients
		if (SACSCCmetrics{CCCrep}.DCpeak_A>=paramsOUT.minDCpeak_CCCtfs)&(SACSCCmetrics{CCCrep}.DCpeak_B>=paramsOUT.minDCpeak_CCCtfs)
			SACSCCmetrics{CCCrep}.CCCtfs = SACSCCmetrics{CCCrep}.DCpeak_AB/(sqrt(SACSCCmetrics{CCCrep}.DCpeak_A*SACSCCmetrics{CCCrep}.DCpeak_B));
		else
			SACSCCmetrics{CCCrep}.CCCtfs = NaN;
		end
		%% USE THIS ONE: Eq. 3 - 10-300 PSD, with noise bias removed
		SACSCCmetrics{CCCrep}.CCCenv=max([0 (SACSCCmetrics{CCCrep}.sums.sumCSDsc_AB_CCCenv-SACSCCmetrics{CCCrep}.sums.sumCSDsc_ABrand_CCCenv)])/ ...
			sqrt((SACSCCmetrics{CCCrep}.sums.sumPSDsc_A_CCCenv-SACSCCmetrics{CCCrep}.sums.sumPSDsc_Arand_CCCenv)* ...
			(SACSCCmetrics{CCCrep}.sums.sumPSDsc_B_CCCenv-SACSCCmetrics{CCCrep}.sums.sumPSDsc_Brand_CCCenv));
		% raw SC peaks
		SACSCCmetrics{CCCrep}.CCCenv2 = (SACSCCmetrics{CCCrep}.SCpeak2_AB-1)/(sqrt((SACSCCmetrics{CCCrep}.SCpeak2_A-1)*(SACSCCmetrics{CCCrep}.SCpeak2_B-1)));
		% 10-300 Hz, bias NOT removed
		SACSCCmetrics{CCCrep}.CCCenv3=SACSCCmetrics{CCCrep}.sums.sumCSDsc_AB_CCCenv/sqrt(SACSCCmetrics{CCCrep}.sums.sumPSDsc_A_CCCenv*SACSCCmetrics{CCCrep}.sums.sumPSDsc_B_CCCenv);
		% 10-CF Hz, noise bias removed
		SACSCCmetrics{CCCrep}.CCCenv4=max([0 (SACSCCmetrics{CCCrep}.sums.sumCSDsc_AB_CCCenv_CF-SACSCCmetrics{CCCrep}.sums.sumCSDsc_ABrand_CCCenv_CF)])/ ...
			sqrt((SACSCCmetrics{CCCrep}.sums.sumPSDsc_A_CCCenv_CF-SACSCCmetrics{CCCrep}.sums.sumPSDsc_Arand_CCCenv_CF)* ...
			(SACSCCmetrics{CCCrep}.sums.sumPSDsc_B_CCCenv_CF-SACSCCmetrics{CCCrep}.sums.sumPSDsc_Brand_CCCenv_CF));
		% adjusted SC peaks
		SACSCCmetrics{CCCrep}.CCCenv5 = (SACSCCmetrics{CCCrep}.SCpeak_AB-1)/(sqrt((SACSCCmetrics{CCCrep}.SCpeak_A-1)*(SACSCCmetrics{CCCrep}.SCpeak_B-1)));
		...
		SACSCCmetrics{CCCrep}.CDscc_usec=findCD_SCC(SACSCCs.SCC_AB_avg,SACSCCs.delays_usec);
		SACSCCmetrics{CCCrep}.CDenv_usec=findCD_SCC(SACSCCs.SUMCOR_AB,SACSCCs.delays_usec);
		SACSCCmetrics{CCCrep}.CDtfs_usec=findCD_SCC(SACSCCs.DIFCOR_AB,SACSCCs.delays_usec);
	end

	%% Book-keeping
	SACSCCfunctions{CCCrep}=SACSCCs;
	if ~isnan(NumDrivenSpikes(2,1))
		SACSCCfunctions{CCCrep}.rand.PSDsc_A=SACSCCs_rand.PSDsc_A;
		SACSCCfunctions{CCCrep}.rand.PSDsc_B=SACSCCs_rand.PSDsc_B;
		SACSCCfunctions{CCCrep}.rand.CSDsc_AB=SACSCCs_rand.CSDsc_AB;
	end

	SACSCCmetrics{CCCrep}.NumDrivenSpikes=NumDrivenSpikes;
	SACSCCmetrics{CCCrep}.AvgRate_sps=AvgRate_sps;

% 	if PLOT_15panel
% 		plot_CCCanal_0(SACSCCfunctions{CCCrep},SACSCCmetrics{CCCrep},paramsOUT)
% 	end
...
CCCenvVEC=zeros(1,paramsOUT.Nreps_CCCavging);
CCCtfsVEC=zeros(1,paramsOUT.Nreps_CCCavging);
for CCCrep=1:paramsOUT.Nreps_CCCavging
	CCCenvVEC(CCCrep)=SACSCCmetrics{CCCrep}.CCCenv;
	CCCtfsVEC(CCCrep)=SACSCCmetrics{CCCrep}.CCCtfs;
end
CCCenvAVG=mean(CCCenvVEC);
CCCtfsAVG=mean(CCCtfsVEC);
CCCenvSTD=std(CCCenvVEC);
CCCtfsSTD=std(CCCtfsVEC);
SACSCCmetrics{1}.CCCenvVEC=CCCenvVEC;
SACSCCmetrics{1}.CCCtfsVEC=CCCtfsVEC;
SACSCCmetrics{1}.CCCenvAVG=CCCenvAVG;
SACSCCmetrics{1}.CCCtfsAVG=CCCtfsAVG;
SACSCCmetrics{1}.CCCenvSTD=CCCenvSTD;
SACSCCmetrics{1}.CCCtfsSTD=CCCtfsSTD;

if PLOT_15panel
	plot_CCCanal_2(SACSCCfunctions,SACSCCmetrics,paramsOUT)
end
