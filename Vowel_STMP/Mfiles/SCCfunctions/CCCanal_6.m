function [SACSCCfunctions,SACSCCmetrics,paramsOUT] = CCCanal_6(SpikeTrains,paramsIN,PLOT_15panel)
% File: CCCanal.m
...
% CCCanal_6: Should now work with a single polarity
%
...
if isempty(SpikeTrains{1,2}) && isempty(SpikeTrains{2,2})
    bothPolarities = 0;
else
    bothPolarities = 1;
end
...
if bothPolarities
    ...
    if isempty(SpikeTrains{2,1}) % we have only A+
        paramsOUT.BOOTSTRAP_Navgs = Navgs(1);
    else % we have a B+ (but no B-)
        paramsOUT.BOOTSTRAP_Navgs=min(Navgs(:,1));
    end
end
for i=1:2
    for j=1:bothPolarities+1
...
for BOOTrep=1:paramsOUT.BOOTSTRAP_Navgs+1
    ...
    % 3) SACpeak (CD delay)
    if isfield(SACSCCmetrics{BOOTrep},'CDscc_usec')
    ...
    if ~isnan(NumDrivenSpikes(2,1)) && bothPolarities
...
if ~isempty(ST_A_minus) % if we have spikes
    bothPolarities=1;
...
if bothPolarities, SAC_A_minus=SAC_A_minus+WindowCORRECTION; end
...
if ~isempty(ST_B_plus)
    ...
    if bothPolarities, PSDsc_B=abs(FFTtemp); end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if bothPolarities, SCC_AB_minus=SCC_AB_minus+WindowCORRECTION; end
    % smooth and AVG correlograms
    ...
    if bothPolarities, CSDsc_AB=abs(FFTtemp); end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
else
    SAC_B_plus=[];
    AvgRate_sps(2,1)=NaN;
...
if bothPolarities
    ...
    SACSCCs=struct('delays_usec',SACdelays_usec,'SAC_A_avg',SAC_A_avg,'SAC_B_avg',SAC_B_avg,'SCC_AB_avg',SCC_AB_avg);
end
return;
