function [SACSCCfunctions,SACSCCmetrics,paramsOUT] = CCCanal_1(SpikeTrains,paramsIN,PLOT_15panel)
% File: CCCanal.m
...
% minimum DCpeak for A and B to compute CCCtfs
	...
	% Number of REPS for Averaging PSD/CSDs for RANDOM SPIKES (Bartlett
	% technique for AVGing Periodograms (Rangayyan, p.290)
	paramsOUT.Nreps_RS_PSDavging=5;
	...
	if ~isnan(NumDrivenSpikes(2,1))
		PSDsc_Ai = zeros(paramsOUT.Nreps_RS_PSDavging,Nfft_psd);
		PSDsc_Bi = zeros(paramsOUT.Nreps_RS_PSDavging,Nfft_psd);
		CSDsc_ABi = zeros(paramsOUT.Nreps_RS_PSDavging,Nfft_psd);
		for RSrep=1:paramsOUT.Nreps_RS_PSDavging
			...
			disp(sprintf('   ... Computing SACs/SCCs for RANDOM spikes (for bias removal): REP=%d of %d ...',RSrep,paramsOUT.Nreps_RS_PSDavging))
			[SACSCCs_rand,AvgRate_rand_sps]=SACSCCanal(STrand_A_plus,STrand_A_minus,STrand_B_plus,STrand_B_minus,paramsOUT);
			
			PSDsc_Ai(RSrep,:)=SACSCCs_rand.PSDsc_A;
			PSDsc_Bi(RSrep,:)=SACSCCs_rand.PSDsc_B;
			CSDsc_ABi(RSrep,:)=SACSCCs_rand.CSDsc_AB;
		end
		PSDsc_Aavg=mean(PSDsc_Ai);
		PSDsc_Bavg=mean(PSDsc_Bi);
		CSDsc_ABavg=mean(CSDsc_ABi);
		
		SACSCCs_rand=rmfield(SACSCCs_rand,'PSDsc_A');
		SACSCCs_rand=rmfield(SACSCCs_rand,'PSDsc_B');
		SACSCCs_rand=rmfield(SACSCCs_rand,'CSDsc_AB');
		SACSCCs_rand.PSDsc_Aavg=PSDsc_Aavg;
		SACSCCs_rand.PSDsc_Bavg=PSDsc_Bavg;
		SACSCCs_rand.CSDsc_ABavg=CSDsc_ABavg;
		SACSCCs_rand.PSDsc_Ai=PSDsc_Ai;
		SACSCCs_rand.PSDsc_Bi=PSDsc_Bi;
		SACSCCs_rand.CSDsc_ABi=CSDsc_ABi;
...
if ~isnan(NumDrivenSpikes(2,1))
	...
	SACSCCmetrics.sums.sumPSDsc_Arand_CCCenv = sum(SACSCCs_rand.PSDsc_Aavg(CCCenv_LOWmod_ind:CCCenv_HIGHmod_ind));
	SACSCCmetrics.sums.sumPSDsc_Brand_CCCenv = sum(SACSCCs_rand.PSDsc_Bavg(CCCenv_LOWmod_ind:CCCenv_HIGHmod_ind));
	SACSCCmetrics.sums.sumCSDsc_ABrand_CCCenv = sum(SACSCCs_rand.CSDsc_ABavg(CCCenv_LOWmod_ind:CCCenv_HIGHmod_ind));
end
...
if ~isnan(NumDrivenSpikes(2,1))
	...
	SACSCCmetrics.sums.sumPSDsc_Arand_0_CCCenv = sum(SACSCCs_rand.PSDsc_Aavg(CCCenv_ZEROmod_ind:CCCenv_HIGHmod_ind));
	SACSCCmetrics.sums.sumPSDsc_Brand_0_CCCenv = sum(SACSCCs_rand.PSDsc_Bavg(CCCenv_ZEROmod_ind:CCCenv_HIGHmod_ind));
	SACSCCmetrics.sums.sumCSDsc_ABrand_0_CCCenv = sum(SACSCCs_rand.CSDsc_ABavg(CCCenv_ZEROmod_ind:CCCenv_HIGHmod_ind));
end
...
if ~isnan(NumDrivenSpikes(2,1))
	...
	SACSCCmetrics.sums.sumPSDsc_Arand_0_CF = sum(SACSCCs_rand.PSDsc_Aavg(CCCenv_ZEROmod_ind:CCCenv_CFmod_ind));
	SACSCCmetrics.sums.sumPSDsc_Brand_0_CF = sum(SACSCCs_rand.PSDsc_Bavg(CCCenv_ZEROmod_ind:CCCenv_CFmod_ind));
	SACSCCmetrics.sums.sumCSDsc_ABrand_0_CF = sum(SACSCCs_rand.CSDsc_ABavg(CCCenv_ZEROmod_ind:CCCenv_CFmod_ind));
end
...
if ~isnan(NumDrivenSpikes(2,1))
	...
	SACSCCmetrics.sums.sumPSDsc_Arand_CCCenv_CF = sum(SACSCCs_rand.PSDsc_Aavg(CCCenv_LOWmod_ind:CCCenv_CFmod_ind));
	SACSCCmetrics.sums.sumPSDsc_Brand_CCCenv_CF = sum(SACSCCs_rand.PSDsc_Bavg(CCCenv_LOWmod_ind:CCCenv_CFmod_ind));
	SACSCCmetrics.sums.sumCSDsc_ABrand_CCCenv_CF = sum(SACSCCs_rand.CSDsc_ABavg(CCCenv_LOWmod_ind:CCCenv_CFmod_ind));
end
...
if ~isnan(NumDrivenSpikes(2,1))
	...
	SACSCCmetrics.sums.sumPSDsc_Arand_0_NYQ = sum(SACSCCs_rand.PSDsc_Aavg(CCCenv_ZEROmod_ind:CCCenv_NYQmod_ind));
	SACSCCmetrics.sums.sumPSDsc_Brand_0_NYQ = sum(SACSCCs_rand.PSDsc_Bavg(CCCenv_ZEROmod_ind:CCCenv_NYQmod_ind));
	SACSCCmetrics.sums.sumCSDsc_ABrand_0_NYQ = sum(SACSCCs_rand.CSDsc_ABavg(CCCenv_ZEROmod_ind:CCCenv_NYQmod_ind));
end
...
if ~isnan(NumDrivenSpikes(2,1))
	SACSCCfunctions.rand.PSDsc_Aavg=SACSCCs_rand.PSDsc_Aavg;
	SACSCCfunctions.rand.PSDsc_Bavg=SACSCCs_rand.PSDsc_Bavg;
	SACSCCfunctions.rand.CSDsc_ABavg=SACSCCs_rand.CSDsc_ABavg;
	SACSCCfunctions.rand.PSDsc_Ai=SACSCCs_rand.PSDsc_Ai;
	SACSCCfunctions.rand.PSDsc_Bi=SACSCCs_rand.PSDsc_Bi;
	SACSCCfunctions.rand.CSDsc_ABi=SACSCCs_rand.CSDsc_ABi;
end
...
% 	paramsOUT.YLIMIT_SClow=.5;
	...
	plot_CCCanal_1(SACSCCfunctions,SACSCCmetrics,paramsOUT)
end
