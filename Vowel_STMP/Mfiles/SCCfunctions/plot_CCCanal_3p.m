function plot_CCCanal_3p(SACSCCfunctions,SACSCCmetrics,paramsIN)
% File: plot_CCCanal(SACSCCfunctions,SACSCCmetrics,paramsIN)
%
% Sept 26, 2008 - updated SAC, DC, tfs to compute several ways - so needed
% to update plot_CCCanal_3, this is the PREVIOUS VERSION (plot_CCCanal_3p)
...
title(sprintf('DIFCOR_A (peak=%.2f %s])',SACSCCmetrics{AVGind}.DCpeak_A,DCpeak_A_TEXT(1:end-2)),'Interpreter','none','FontSize',8);
set(gca, 'Box', 'off', 'TickDir', 'out');
...
plot(XLIMIT_delay*[-1 1],zeros(1,2),'-k');hold off
xlim(XLIMIT_delay*[-1 1]); ylim(YLIMIT_DC*[-1 1]);
title(sprintf('DIFCOR_B (peak=%.2f %s])',SACSCCmetrics{AVGind}.DCpeak_B,DCpeak_B_TEXT(1:end-2)),'Interpreter','none','FontSize',8);
set(gca, 'Box', 'off', 'TickDir', 'out');
...
title(sprintf('%s=%.2f %s; %.2f(%.2f)]',texlabel('rho_{TFS}'),SACSCCmetrics{AVGind}.CCCtfs,CCCtfsTEXT(1:end-2), ...
	SACSCCmetrics{AVGind}.CCCtfsAVG,SACSCCmetrics{AVGind}.CCCtfsSTD),'FontSize',10,'Color','red');
text(0.02,1,sprintf('DIFCOR_AB (peak=%.2f %s])',SACSCCmetrics{AVGind}.DCpeak_AB,DCpeak_AB_TEXT(1:end-2)),'units','norm', ...
...
for repIND=1:Nreps
	...
	plot(SACSCCfunctions{repIND}.delays_usec/1000,SACSCCfunctions{repIND}.SUMCORadj_A,'Color',REPcolors{repIND}); 
	SCpeak_A_TEXT=sprintf('%s%.2f, ',SCpeak_A_TEXT,SACSCCmetrics{repIND}.SCpeaks_A(SCpeak_TOUSEindex));
end
...
title(sprintf('SUMCOR_A (peak=%.2f %s] ["%s"] )',SACSCCmetrics{AVGind}.SCpeaks_A(SCpeak_TOUSEindex),SCpeak_A_TEXT(1:end-2),paramsIN.SCpeak_TOUSE), ...
...
for repIND=1:Nreps
	...
	plot(SACSCCfunctions{repIND}.delays_usec/1000,SACSCCfunctions{repIND}.SUMCORadj_B,'Color',REPcolors{repIND}); 
	SCpeak_B_TEXT=sprintf('%s%.2f, ',SCpeak_B_TEXT,SACSCCmetrics{repIND}.SCpeaks_B(SCpeak_TOUSEindex));
end
...
plot(zeros(1,2),YLIMIT_SC*[0 1],'--k'); hold off
xlim(XLIMIT_delay*[-1 1]); ylim([YLIMIT_SClow YLIMIT_SC]);
title(sprintf('SUMCOR_B (peak=%.2f %s])',SACSCCmetrics{AVGind}.SCpeaks_B(SCpeak_TOUSEindex),SCpeak_B_TEXT(1:end-2)),'Interpreter','none','FontSize',8);
set(gca, 'Box', 'off', 'TickDir', 'out');
...
for repIND=1:Nreps
	...
	plot(SACSCCfunctions{repIND}.delays_usec/1000,SACSCCfunctions{repIND}.SUMCORadj_AB,'Color',REPcolors{repIND}); 
...
xlim(XLIMIT_delay*[-1 1]);  ylim([YLIMIT_SClow YLIMIT_SC]);
set(gca, 'Box', 'off', 'TickDir', 'out');
title(sprintf('SUMCOR_AB (peak=%.2f %s])',SACSCCmetrics{AVGind}.SCpeaks_AB(SCpeak_TOUSEindex),SCpeak_AB_TEXT(1:end-2)),'Interpreter','none','FontSize',8);
