function UnitModel_EHIN_CoincDet()
% File: UnitModel_EHIN_CoincDet.m
% model version, based on UnitLook_EHIN_CoincDet2.m
% This code loops through all freq pairs to calculate SCC and rho.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
close all
ROOT_dir='C:\Research\MATLAB\Vowel_STMP';
data_dir=fullfile(ROOT_dir,'ModelData');
unitdata_dir=fullfile(data_dir,'UNITSdata');
eval(['cd ''' fullfile(data_dir) ''''])
...
TrackNum=1;
UnitNum=1;  % hard-code these (as if there's only one unit)

% LOAD CALCS if available to speed up figure design
SAVECALCSfilename=sprintf('UnitModel_EHIN_CF%d.mat',CF);
loadBOOL=0;
...
if ~loadBOOL
    % Run auditory nerve model here
    
    
    % Create a structure containing:
    % DataList.CALIB.CalibFile_ToUse
    % unit.Info.Q10
    %          .BF_kHz
    %          .Threshold_dBSPL
    %          .SR_sps
    % unit.EHvN_reBF
    % unit.EHvN_reBF_simFF
    %     .EHvN_reBF_simFF.F1{HarmonicsIND,PolarityIND}.TimeFact(1,NSCC_BFinds{SCCind}(1))
    % what else? ...
    ...
    for ATTind=1:length(Nattens_dB)
        for FeatIND=FeatINDs
            if ~isnan(SMP_NSCC_0delay{1,ATTind}(FeatIND))
                plot(yTEMP.levels_dBSPL+FeatureLevels_dB(FeatIND),SMP_NSCC_Oct_at_Rho_pos{FeatIND-2,ATTind},'Marker',FEATmarkers{FeatIND},'Color',ATTENcolors{ATTind},'LineStyle','none')
                hold on
                plot(yTEMP.levels_dBSPL+FeatureLevels_dB(FeatIND),SMP_NSCC_Oct_at_Rho_neg{FeatIND-2,ATTind},'Marker',FEATmarkers{FeatIND},'Color',ATTENcolors{ATTind},'LineStyle','none')
            end
        end
    end
    ...
    for ATTind=1:length(Nattens_dB)
        for FeatIND=FeatINDs
            if ~isnan(SMP_NSCC_CD{1,ATTind}(FeatIND))
                plot(yTEMP.levels_dBSPL+FeatureLevels_dB(FeatIND),SMP_NSCC_CD_at_Oct_pos{FeatIND-2,ATTind}/1000, ...
                    'Marker',FEATmarkers{FeatIND},'Color',ATTENcolors{ATTind},'LineStyle','none')
                plot(yTEMP.levels_dBSPL+FeatureLevels_dB(FeatIND),SMP_NSCC_CD_at_Oct_neg{FeatIND-2,ATTind}/1000, ...
                    'Marker',FEATmarkers{FeatIND},'Color',ATTENcolors{ATTind},'LineStyle','none')
                hold on
            end
        end
