function PIC=concatPICS_STMP(picNums,excludeLines)
...
if sum(strcmp(PIC.TEMPLATE,{'EHrBFi','EHvNrBFi','TrBFi','SACrlv'}))
   PIC.CONDind=mod(min(setdiff(1:excludeLines{1}(end),excludeLines{1})),PIC.x.Stimuli.nstim);
   if (PIC.CONDind==0)
      PIC.CONDind=PIC.x.Stimuli.nstim;
   end
...
elseif sum(strcmp(PIC.TEMPLATE,{'EHrBFi','EHvNrBFi'}))  % Interleaved vowels
   % Troughs and Formants
...
if ~isfield(PIC,'BF_Hz')
   UnitFile=dir(sprintf('UNITinfo/unitINFO.%d.%.02d.mat',PIC.x.General.track,PIC.x.General.unit));
   if ~isempty(UnitFile)
      eval(['load ' sprintf('UNITinfo/unitINFO.%d.%.02d.mat',PIC.x.General.track,PIC.x.General.unit)])
      if ~isempty(unitINFO.Info.BF_kHz)
         PIC.BF_Hz=unitINFO.Info.BF_kHz*1000;
      end
   end
end
