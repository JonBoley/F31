	function [drat,srat,abval,npts,nralvs]=ratep(str)
% RATEP('a4-7,15[s][f]') - computes and displays  rate versus level functions
% for driven and spontaneous rates from the files specified, which should
% be in standard lab format.
...
% NOTE: for convenience, call this as rate.
...
% when there is a cursor display line.
		...
		while displayloop==0               % #6 displayloop
		   for j=1:nplots
            xax = [100000 -100000];
            if logabs == 0
                 plot(abda(1:nlavg(j),j), drda(1:nlavg(j),j), strcat(plcolor(j),t(j),'-'), ...
			       abda(1:nlavg(j),j), spda(1:nlavg(j),j), strcat(plcolor(j),t(j),'--'))
	          else
		         semilogx(abda(1:nlavg(j),j), drda(1:nlavg(j),j), strcat(plcolor(j),t(j),'-'), ...
			       abda(1:nlavg(j),j), spda(1:nlavg(j),j), strcat(plcolor(j),t(j),'--'))
    ...
    '\nFn-filter, Hn-shift n dB right, Cursor, type 2 anal(@), Sa5-superimpose, Na5-next: ','s');
		   ...
		   elseif what(1)=='H' | what(1)=='h'
			  ...
			  abda(1:nlavg(nplots),nplots) = abda(1:nlavg(nplots),nplots)+nsh;
			  legend(nplots) = cellstr(shleg(char(legend(nplots)), nsh));
		   ...
		   elseif what(1)=='2' | what(1)=='@'
			  ...
			  while istypeII==1
				  [nthr,spont,nd1,nd2,dynr,rmax,hislope,hioff,nhi1,nhi2] = ...
				      getII(drda(1:nlavg(nplots),nplots),abda(1:nlavg(nplots),nplots),...
					  thrin,abmaxin,abminin);
				  if nthr>0
					  hold on
					  axlim=axis;
					  line([abda(nthr,nplots) abda(nthr,nplots)], ...
					     [axlim(3) axlim(3)+0.2*(axlim(4)-axlim(3))],'Color','m');
					  line([abda(1,nplots) abda(nthr,nplots)],...
					     [spont spont],'Color','m');
					  plot([abda(nd1,nplots) abda(nd2,nplots)], ...
					       [drda(nd1,nplots) drda(nd2,nplots)], 'm*')
					  text(abda(nd1,nplots)+0.02*(axlim(2)-axlim(1)), ...
					      drda(nd1,nplots), ...
					      sprintf('dyn. range = %g dB',dynr),'Color','m')
					  if nhi1>0
						  hiy= hioff + hislope*abda(nhi1:nhi2,nplots);
						  plot(abda(nhi1:nhi2,nplots), hiy, 'm-')
						  text(abda(round((nhi1+nhi2)/2),nplots)+0.02*(axlim(2)-axlim(1)), ...
						     drda(round((nhi1+nhi2)/2),nplots), ...
       ...
       'Date,trun     P.No.  dynrng   rmax    slope  norm_slope slope range\n');
				     ...
				     fprintf(fid, ...
					     ...
					     nobl(titline),simplfy(char(legend(nplots))),dynr, ...
						 rmax,hislope,hislope/rmax, ...
					     abda(nhi1,nplots),abda(nhi2,nplots));
					 elseif what=='r' | what=='R'
					    [thrin,abmaxin,abminin]=showme;
					    erasem(abda,drda,nthr,nplots,spont,nd1,nd2,dynr, ...
...
%  Frequency range in left (index=1) or right (index=6) channel
		...
		if (pb.stim_par(1)==0 | pb.stim_par(1)==9) & pb.stim_par(2)==-1
			index = 1;
		elseif (pb.stim_par(3)==0 | pb.stim_par(3)==9) & pb.stim_par(4)==-1
			index = 6;
