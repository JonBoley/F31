...
%%################################################################################################
function do_Ians_PST()
global FIG PIC FeaturesText

%%%% Plot PST Histogram
figure(111); clf
set(gcf,'pos',[5         476        1391         420])
plot(PIC.PST.pst_X_sec, PIC.PST.pst_Y_sps, 'k');
hold on
plot(PIC.PST.pst_X_sec(PIC.PST.drivenPSTinds), PIC.PST.pst_Y_sps(PIC.PST.drivenPSTinds), 'b');
xlim([0 FIG.raster.xmax]);
...
TU=getTrackUnit(getFileName(PIC.nums));

ConditionString = sprintf('BF = %.3f kHz\nThreshold = %.f dB SPL\nTone Level= %.f dB SPL\n%d reps\nPST binwidth = %.f usec', ...
   PIC.BF_Hz/1000, ...
   DataList.Units{TU(1),TU(2)}.Info.Threshold_dBSPL, ...
   PIC.x.Stimuli.Condition.Level_dBSPL, ...
   PIC.x.Stimuli.fully_presented_lines, ...
   PIC.PST.params.binWidth_sec*1e6)

text(.9,.9,ConditionString,'Horiz','right','Vertic','top','units','norm')
