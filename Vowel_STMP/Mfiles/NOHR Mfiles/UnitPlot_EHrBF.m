function UnitPlot_EHrBF(ExpDate,UnitName)
% File: UnitPlot_EHrBF.m
% Modified Date: 07Jan2005 - incorporated EHrBFi here
%
...
disp(sprintf('Plotting (EHreBF) Experiment: ''%s''; Unit: %d.%02d',ExpName,TrackNum,UnitNum))
   ...
   for HarmonicsIND=1:2
      for PolarityIND=1:2
         ...
         if ~isempty(yTEMP)
            ...
            ht1=title(sprintf('Exp: %s; Unit: %s [BF=%.4f kHz, Thr= %.f dB atten]\n[UnitPlot_EHreBF]   Feature: %s (plotting: %s);    Formants at Harmonics: %s;    Invert Polarity: %s', ...
