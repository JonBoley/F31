function UnitPlot_EHrBFi_simFF(ExpDate,UnitName)
% File: UnitPlot_EHrBFi_simFF.m
% Modified Date: 122804 (M Heinz)
% Modified From: UnitPlot_EHrBF_simFF.m to use Interleaved Data
%
...
disp(sprintf('Plotting (EHreBFi_simFF) Experiment: ''%s''; Unit: %d.%02d',ExpName,TrackNum,UnitNum))
...
UnitFeats=fieldnames(unit.EH_reBFi);
   ...
   for HarmonicsIND=1:2
      for PolarityIND=1:2
         ...
         if ~isempty(yTEMP)
            ...
            ht1=title(sprintf('Exp: %s; Unit: %s [BF=%.4f kHz, Thr= %.f dB atten]\n[UnitPlot_EHreBFi_SIMreFF]   Feature: %s (plotting: %s);    Formants at Harmonics: %s;    Invert Polarity: %s', ...
