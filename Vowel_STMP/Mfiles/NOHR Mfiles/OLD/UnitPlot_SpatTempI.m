function UnitPlot_SpatTempI(ExpDate,UnitName)
% File: UnitPlot_SpatTempI.m
% Modified Date: 121304/122804 (M Heinz)
% Modified From: UnitPlot_SpatTemp.m to use Interleaved Data
%
...
%
% 11/2/04 TO DO
% 1) only plots F1{1,1} for now, need to add rest later
%
...
disp(sprintf('Plotting (Spatio-Temporal: EHrBFi) Experiment: ''%s''; Unit: %d.%02d',ExpName,TrackNum,UnitNum))
...
if isempty(ddd) %updateBFi
   UnitAnal_EHrBFi_simFF(ExpDate,UnitName,0);
end
eval(['load ''' fullfile('UNITSdata',UnitFileName) ''''])

% Make sure there is EH_reBFi data  %updateBFi
if ~isfield(unit,'EH_reBFi')
   disp(sprintf('*** No EH_reBFi data for this unit!!!'))
   beep
   return;
end

% If EH_reBFi_simFF analysis is not completed, run here
if ~isfield(unit,'EH_reBFi_simFF')
   UnitAnal_EHrBFi_simFF(ExpDate,UnitName,0);   
   eval(['load ''' fullfile('UNITSdata',UnitFileName) ''''])
end
         
%%%% Find number of features to plot (F1, T1, ...)
%%%% Find all levels and BFs to plot
NUMcols=4;
NUMrows=0;
levels_dBSPL=[]; BFsTEMP_kHz=[];
if isfield(unit,'EH_reBFi_simFF')
   EHfeats=fieldnames(unit.EH_reBFi_simFF);
   ...
   for FeatIND=FeatINDs
      for HarmonicsIND=1:2
         for PolarityIND=1:2
            eval(['yTEMP=unit.EH_reBFi_simFF.' FeaturesText{FeatIND} '{HarmonicsIND,PolarityIND};'])   %updateBFi
            if ~isempty(yTEMP)
...
YLIMocts=1;  % Number of octaves to show on oridinate
clear BFsTEMP_kHz;
   ...
   if isfield(unit,'EH_reBFi_simFF')
      for FeatIND=FeatINDs
         ...
         for HarmonicsIND=1:1
            for PolarityIND=1:1
               eval(['yTEMP=unit.EH_reBFi_simFF.' FeaturesText{FeatIND} '{HarmonicsIND,PolarityIND};'])
               if ~isempty(yTEMP)
                  %%%% EH_reBFi plots
