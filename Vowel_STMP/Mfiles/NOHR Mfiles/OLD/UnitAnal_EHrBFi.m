function UnitAnal_EHrBFi(ExpDate,UnitName,PLOTyes)
% File: UnitAnal_EHrBFi.m
% Modified Date: 122804 (M Heinz)
% Modified From: UnitAnal_EHrBF.m to use Interleaved Data
%
% LATER: consolidate with EHrBF (see "updateBFi")
%    - basic changes only: BF-->BFi
%    - but inefficient, as setup now, because concatNOHRpics reloads same pictures over and over!!!!
...
if (TrackNum>NumTracks)|(UnitNum>NumUnits)
   ...
   if isempty(DataList.Units{TrackNum,UnitNum})
      error(sprintf('Unit Number: %d.%02d does not exist!!!!!',TrackNum,UnitNum))
   elseif ~isfield(DataList.Units{TrackNum,UnitNum},'EH_reBFi') %updateBFi
      error(sprintf('"EH_reBFi" data does not exist for Unit Number: %d.%02d',TrackNum,UnitNum))
   end
end
   ...
   for HarmonicsIND=1:2
      for PolarityIND=1:2
         ...
         if ~isempty(yTEMP)
            ...
            eval(['unit.EH_reBFi.' FeaturesText{FeatIND} '{HarmonicsIND,PolarityIND}=yTEMP;']) %updateBFi
...
%%%%%%%%%%%% Plot EH_reBFi data
if PLOTyes
   UnitPlot_EHrBFi(ExpDate,UnitName)
end