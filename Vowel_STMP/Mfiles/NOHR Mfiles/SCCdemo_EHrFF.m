function [CD_us,NSCC_0,NSCC_CD,NSCC_HLCD,NSCC_max]=SCCdemo_EHrFF(picNums1,picNums2,excludeLines1,excludeLines2,HLCD_us,SCCtitles,calcSAC)
% M.Heinz 25Sep2004.  Modified from SCCdemo_TrFF.m
%
% **********************************************
% ********* THIS IS BASICALLY THE SAME as SCCdemo_TrFF, doesn't need to be different, just quicker in the RUSH!!!!
% LATER: make one general SSCdemo_reFF.m Mfile
% **********************************************
%
...
if ~exist('SCCtitles','var')
   SCCtitles={'UNIT 1','UNIT 2','SCCdemo_EHreFF'};
end
...
% 5) Setup all SCC analysis into UnitsPlot (may need some smart delay offset to see effect???)
...
diff_LtoR=diff(trifilt(NSCC,TFiltWidth));
diff_RtoL=diff(fliplr(trifilt(NSCC,TFiltWidth)));
...
LocalMaxDelays=intersect(LocalMaxDelays1,NSCCdelays(find(NSCC>=1)));
...
% WRONG!!!!!!!!!!!! 2/5/05 normalization to rate is to multiply by: SCC=NSCC*r1*r2*BINwidth
...
if strcmp(SCCtitles{3},'SCCdemo_EHreFF')
...
xlabel('Delay (\mus)  [black circle: HLCD; red triangle: CD]','Interpreter','tex')
ylabel('Normalized SAC')
hold off
xlim([-XLIMIT XLIMIT])
ylim([0 Ymax])
