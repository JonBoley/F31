...
%       if abs(log2(unit.Tone_reBF_simFF.BFs_kHz(BFind2)/unit.Tone_reBF_simFF.BFs_kHz(BF_INDEX))-SCC_octOFFSET)>0.05
%          %                      warndlg('The two BFs used for the first (BF+Xoctaves) SCC are more than 0.05 octaves different than the desired distance')
%          beep
%          disp('*** WARNING ***: The two BFs used for the first (BF+Xoctaves) SCC are more than 0.05 octaves different than the desired distance')
%       end
...
%          beep
%          disp('*** WARNING ***: The two BFs used for the second (BF-Xoctaves) SCC are more than 0.05 octaves different than the desired distance')
%       end
      NSCC_BFinds{SCCind}=[BF_INDEX BFind2];
      SCC_allBFinds=[SCC_allBFinds NSCC_BFinds{SCCind}];
...
%       title(sprintf('     Exp%s, Unit %s: BF=%.2f kHz, Thr=%.f dB SPL, SR=%.1f sps, Q10=%.1f\n%s @ %.f dB SPL,   Harm: %d, Polarity: %d', ...
%          ExpDate,UnitName,unit.Info.BF_kHz,unit.Info.Threshold_dBSPL,unit.Info.SR_sps,unit.Info.Q10,'TONE', ...
%          unit.Tone_reBF_simFF.levels_dBSPL(LEVind),HarmonicsIND,PolarityIND),'units','norm','pos',[.1 1 0],'HorizontalAlignment','left')
%       xlim(XLIMITS_scc)
%       PLOThand=eval(['h' num2str(PLOTnum)]);
%       set(PLOThand,'YTick',YTICKS,'YTickLabel',YTICKS)
%       ylim(YLIMITS)  % Same Ylimits for all plots
...
%    end
   ...
   %%%%%%%%%%%%%%%%%%%%%
   LEGXleft=.05; LEGYtop=0.85; LEGYstep=0.1; LEGXsymbOFFset=0.05;
   FeatNum=0;
   for FeatIND=FeatINDs
      FeatNum=FeatNum+1;
