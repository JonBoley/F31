function UnitLook_EHIN(UnitName)
% File: UnitLook_EHIN.m
%
...
for ATTind=1:length(Nattens_dB)
	...
	if ~isnan(SensitivitySlopes_nscc0{1}(ATTind))
		...
		plot(xdata,ydata,'Marker','none','Color',ATTENcolors{ATTind},'LineStyle','-')
		hold on
		LEGtext{length(LEGtext)+1}=sprintf('%.f dB (%.3f)',Nattens_dB(ATTind),SensitivitySlopes_nscc0{1}(ATTind));  % 1/dB
	end
end
for ATTind=1:length(Nattens_dB)
	for FeatIND=FeatINDs
...
for ATTind=1:length(Nattens_dB)
	...
	if ~isnan(SensitivitySlopes_nsccPEAK{1}(ATTind))
		...
		plot(xdata,ydata,'Marker','none','Color',ATTENcolors{ATTind},'LineStyle','-')
		hold on
		LEGtext{length(LEGtext)+1}=sprintf('%.f dB (%.3f)',Nattens_dB(ATTind),SensitivitySlopes_nsccPEAK{1}(ATTind));  % 1/dB
	end
end
for ATTind=1:length(Nattens_dB)
	for FeatIND=FeatINDs
...
ylabel(sprintf('Peak NSCC (at CD)\n [Deng and Geisler 1987]'))
xlabel('Feature Level (dB SPL)')
hleg=legend(LEGtext,1);
set(hleg,'FontSize',SMPlegFontSize)
hold off
...
title('Peak Normalized Cross Correlation - DENG AND GEISLER 1987')
ylabel('SMP Slope (1/dB)')
xlabel('Noise Attenuation (dB)')
