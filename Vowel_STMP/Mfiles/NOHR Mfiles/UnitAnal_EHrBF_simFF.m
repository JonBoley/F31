function UnitAnal_EHrBF_simFF(ExpDate,UnitName,PLOTyes)
...
disp(sprintf('Processing (EHreBF_simFF) Experiment: ''%s''; Unit: %d.%02d',ExpName,TrackNum,UnitNum))
...
for FeatIND=FeatINDs  % Step through each Feature we have data for
   ...
   eval(['unit.EH_reBF_simFF.' FeaturesText{FeatIND} '=cell(length(FormsAtHarmonicsText),length(InvertPolarityText));'])   
   
   for HarmonicsIND=1:2
      for PolarityIND=1:2
         
         eval(['yTEMP0=unit.EH_reBF.' FeaturesText{FeatIND} '{HarmonicsIND,PolarityIND};'])
         if ~isempty(yTEMP0)
            ...
            eval(['unit.EH_reBF_simFF.' FeaturesText{FeatIND} '{HarmonicsIND,PolarityIND}=yTEMP;'])
...
if PLOTyes
   UnitPlot_EHrBF_simFF(ExpDate,UnitName)
end
