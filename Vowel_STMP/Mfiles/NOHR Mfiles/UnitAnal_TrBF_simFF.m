function UnitAnal_TrBF_simFF(ExpDate,UnitName,PLOTyes)
% File: UnitAnal_TrBF_simFF.m
% Date: 28Sep2004 (M. Heinz) (modified from UnitAnal_TrBF.m)
% For: NOHR Experiments
%
% ExpDate: e.g., '070804' (converted later)
% UnitName: '3.07' (converted later)
%
% Performs Tone_reBF_simFF analysis for a given experiment and unit.  Loads 'DataList' file, and unit file.
% UnitPlot_TrBF_simFF.m plots results from this analysis
%
...
disp(sprintf('Processing  (TreBF_simFF) Experiment: ''%s''; Unit: %d.%02d',ExpName,TrackNum,UnitNum))
...
unit.Tone_reBF_simFF.BFs_kHz=NaN+zeros(size(unit.Tone_reBF.freqs_kHz));
unit.Tone_reBF_simFF.levels_dBSPL=unit.Tone_reBF.levels_dBSPL;
unit.Tone_reBF_simFF.picNums=unit.Tone_reBF.picNums;
unit.Tone_reBF_simFF.excludeLines=unit.Tone_reBF.excludeLines;

unit.Tone_reBF_simFF.rate=NaN+zeros(size(unit.Tone_reBF.picNums));
unit.Tone_reBF_simFF.synch=NaN+zeros(size(unit.Tone_reBF.picNums));
unit.Tone_reBF_simFF.phase=NaN+zeros(size(unit.Tone_reBF.picNums));
unit.Tone_reBF_simFF.RaySig=NaN+zeros(size(unit.Tone_reBF.picNums));

for freqIND=1:length(unit.Tone_reBF.freqs_kHz)
   for levelIND=1:length(unit.Tone_reBF_simFF.levels_dBSPL)
      if ~isempty(unit.Tone_reBF_simFF.picNums{levelIND,freqIND})
         
         % Gather spikes from relevant pictures
         PIC=concatPICS_NOHR(unit.Tone_reBF_simFF.picNums{levelIND,freqIND},unit.Tone_reBF_simFF.excludeLines{levelIND,freqIND});

         % Shift spikes and frequencies to simulate shifted-BF neuron with stimulus at nominal-BF
         PIC=simFF_PICshift(PIC);
         unit.Tone_reBF_simFF.BFs_kHz(freqIND)=PIC.BF_Hz/1000;
         
         % Calc Synch and Phase from PERIOD histogram
         PIC=calcSynchRate_PERhist(PIC);
         
         %          unit.Tone_reBF_simFF.rate(levelIND,freqIND)=PIC.PERhist.NumDrivenSpikes/PIC.x.Stimuli.fully_presented_lines/ ...
         %             diff(PIC.PERhist.params.PERhist_window_sec);  
         unit.Tone_reBF_simFF.rate(levelIND,freqIND)=PIC.SynchRate_PERhist.SynchRate_PERhist(1);
         unit.Tone_reBF_simFF.synch(levelIND,freqIND)=PIC.SynchRate_PERhist.FeatureSynchs;
         unit.Tone_reBF_simFF.phase(levelIND,freqIND)=PIC.SynchRate_PERhist.FeaturePhases;
         unit.Tone_reBF_simFF.RaySig(levelIND,freqIND)=PIC.SynchRate_PERhist.FeatureRaySig;
...
%%%%%%%%%%%% Plot Tone_reBF_simFF data
if PLOTyes
   UnitPlot_TrBF_simFF(ExpDate,UnitName)
end