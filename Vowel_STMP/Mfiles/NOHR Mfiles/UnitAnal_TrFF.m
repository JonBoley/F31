function UnitAnal_TrFF(ExpDate,UnitName,PLOTyes)
% File: UnitAnal_TrFF.m
...
%
% Performs Tone_reFF analysis for a given experiment and unit.  Loads 'DataList' file, and unit file.
% UnitPlot_TrFF.m plots results from this analysis
%
...
disp(sprintf('Processing  (TreFF) Experiment: ''%s''; Unit: %d.%02d',ExpName,TrackNum,UnitNum))
...
if (TrackNum>NumTracks)|(UnitNum>NumUnits)
   ...
   if isempty(DataList.Units{TrackNum,UnitNum})
      error(sprintf('Unit Number: %d.%02d does not exist!!!!!',TrackNum,UnitNum))
   elseif ~isfield(DataList.Units{TrackNum,UnitNum},'Tone_reFF')
      error(sprintf('"Tone_reFF" data does not exist for Unit Number: %d.%02d',TrackNum,UnitNum))
   end
end
...
unit.Tone_reFF.rate=NaN+zeros(size(unit.Tone_reFF.picNums));
unit.Tone_reFF.synch=NaN+zeros(size(unit.Tone_reFF.picNums));
unit.Tone_reFF.phase=NaN+zeros(size(unit.Tone_reFF.picNums));
unit.Tone_reFF.RaySig=NaN+zeros(size(unit.Tone_reFF.picNums));

for freqIND=1:length(unit.Tone_reFF.freqs_kHz)
   for levelIND=1:length(unit.Tone_reFF.levels_dBSPL)
      if ~isempty(unit.Tone_reFF.picNums{levelIND,freqIND})
         
         % Gather spikes from relevant pictures
         PIC=concatPICS_NOHR(unit.Tone_reFF.picNums{levelIND,freqIND},unit.Tone_reFF.excludeLines{levelIND,freqIND});
         % Calc Synch and Phase from PERIOD histogram
         PIC=calcSynchRate_PERhist(PIC);

         %          unit.Tone_reFF.rate(levelIND,freqIND)=PIC.PERhist.NumDrivenSpikes/PIC.x.Stimuli.fully_presented_lines/ ...
         %             diff(PIC.PERhist.params.PERhist_window_sec);  
         unit.Tone_reFF.rate(levelIND,freqIND)=PIC.SynchRate_PERhist.SynchRate_PERhist(1);

         unit.Tone_reFF.synch(levelIND,freqIND)=PIC.SynchRate_PERhist.FeatureSynchs;
         unit.Tone_reFF.phase(levelIND,freqIND)=PIC.SynchRate_PERhist.FeaturePhases;
         unit.Tone_reFF.RaySig(levelIND,freqIND)=PIC.SynchRate_PERhist.FeatureRaySig;
...
%%%%%%%%%%%% Plot Tone_reFF data
if PLOTyes
   UnitPlot_TrFF(ExpDate,UnitName)
end