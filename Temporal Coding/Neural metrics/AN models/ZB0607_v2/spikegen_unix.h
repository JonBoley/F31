...
{  
    ...
    T = totalstim * tdres;  /* Total duration of the rate function */
    Nout = 0;
	...
	endOfLastDeadtime = log(drand48()) / synout[0] + dead;  /* End of last deadtime before t=0 */
	...
	unitRateIntrvl = -log(drand48()) /tdres;
	...
	for (j=0; j<nrep; ++j)  /* Loop through "stimulus" repetitions */
		{
		for (; (k<totalstim) && (countTime<T); ++k, countTime+=tdres, 
			...
			if (synout[k]>0.0)  /* Nothing to do for non-positive rates, i.e. Xsum += 0 for non-positive rates. */
				...
				if ( Xsum >= unitRateIntrvl )  /* Spike occurs when time-warping sum exceeds interspike "time" in unit-rate process */
					...
					unitRateIntrvl = -log(drand48()) /tdres;  /* Next interspike "time" in unit-rate process */
					Xsum = 0;
		...
		for (; (Nout>0)&&(sptime[Nout-1]>T); --Nout)  sptime[Nout-1] = mxGetNaN();
