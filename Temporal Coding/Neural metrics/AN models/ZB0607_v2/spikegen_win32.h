...
{  
    ...
    seed = time(NULL);
    gsl_rng_set(r, (unsigned long int)time((time_t *)NULL));
	...
	endOfLastDeadtime = log(gsl_rng_uniform(r)) / synout[0] + dead;  /* End of last deadtime before t=0 */
	...
	unitRateIntrvl = -log(gsl_rng_uniform(r)) /tdres;
		...
		for (; (k<totalstim) && (countTime<DT); ++k, countTime+=tdres, 
			...
			if (synout[k]>0.0)  /* Nothing to do for non-positive rates, i.e. Xsum += 0 for non-positive rates. */
				...
				if ( Xsum >= unitRateIntrvl )  /* Spike occurs when time-warping sum exceeds interspike "time" in unit-rate process */
					...
					unitRateIntrvl = -log(gsl_rng_uniform(r)) /tdres;  /* Next interspike "time" in unit-rate process */
					Xsum = 0;
