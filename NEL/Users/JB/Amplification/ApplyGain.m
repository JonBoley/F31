...
plotYes=0;
if nargin<1  % if no input, just show gain settings
    Fs=22e3;
...
switch strategy
    case 'linear'
        ...
        InsertionGain = NAL_IG;
        output = input * 10^(InsertionGain/20);
    ...
    case {'nonlinear_quiet','nonlinear_noise'}
        ...
        InputSPL = MaxSPL + 20*log10(InputRMS) - atten; 
        ...
        if all(dBLoss==[16,16,18,20,9,9,9])
            ...
            LTASS_dBFS  = [-24.08, -42.37]; % 1/3-octave levels at 500Hz & 4kHz
            LTASS_RMS = -11.9310;
            LTASS_dBSPL = 60 + (LTASS_dBFS-LTASS_RMS); % assume RMS at 60dB SPL
            PredictedREAR = LTASS_dBSPL + PrescribedGain;
            adjustment = TargetREAR_60dBSPL(indx,:) - PredictedREAR;
            PrescribedGain = PrescribedGain + adjustment;
            PrescribedGain = max(0,PrescribedGain); % minimum 0dB gain
            
            if max(PrescribedGain)>0
...
% [in_quiet_band1,in_quiet_band2;
%  in_noise_band1,in_noise_band2;]
TKgain_out = [TKgain(1,1),TKgain(1,end);TKgain(2,1),TKgain(2,end)];
