function [tmplt,DAL,stimulus_vals,units,errstr] = EHrlv_LTASS_template(fieldname,stimulus_vals,units)
   ...
   if ~BADstim   
      ...
      if (exist(stimulus_vals.Inloop.Compiled_FileName,'file') ~= 0)
         ...
         if BASELINE_Fs~=BASENOISE_Fs
            nelerror('In EHrlv_LTASS_template: BASE sampling rates dont match between EH and NOISE!!');
         end
      else
         Llist = {};
         Rlist = {};
...
%    stimulus_vals.Inloop.Computed_Attenuation_dB          = Inloop.params.attens;
   ...
   DAL.short_description   = 'EHrlvLTASS';
...
str{1} = sprintf('RLF_LTASS %dreps:', p.repetitions);
