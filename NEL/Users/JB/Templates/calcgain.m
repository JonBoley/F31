function gainStruct=calcgain(input,Fs,MaxSPL,atten,audiogram,freqs_Hz,strategy)
...
for i=1:3
    ...
    case {1,'linear'}
        ...
        gainStruct.linear.gain_dB = NAL_IG;
        gainStruct.linear.f_Hz = freqs_Hz_std;
%         output = FIRgain(input,NAL_IG,freqs_Hz_std,Fs);
    ...
    case {2,'nonlinear_quiet',3,'nonlinear_noise'}
        ...
        if all(dBLoss==[16,16,18,20,9,9,9])
            ...
            switch strategy
            case {2,'nonlinear_quiet'}
                gainStruct.nonlinear.quiet.gains = PrescribedGain;
            case {3,'nonlinear_noise'}
                gainStruct.nonlinear.noise.gains = PrescribedGain;
            end
            
            % apply gain to 2 bands (between F1 & F2)
