...
if (exist('stimulus_vals','var') == 1)
    ...
    HearingAids=[];
    if strcmp(stimulus_vals.Inloop.HearingAid_None,'yes'), HearingAids(length(HearingAids)+1)=0;  end
    if strcmp(stimulus_vals.Inloop.HearingAid_Linear,'yes'), HearingAids(length(HearingAids)+1)=1;  end
    if strcmp(stimulus_vals.Inloop.HearingAid_Nonlinear,'yes'), HearingAids(length(HearingAids)+1)=2;  end
    % only use 3 in noise (never specify it here)
    if isempty('HearingAids')
        HearingAids(length(HearingAids)+1)=0; % default to none
        stimulus_vals.Inloop.HearingAid_None='yes';
    end
    
    %%%%%%%% Generate Lists here (fill in values for all REPS)
    featureIND_List=zeros(1,length(Features)*length(OctShifts)*length(Attens_dB)*length(HearingAids));
    Computed_FeatureTarget_Hz_List=zeros(1,length(Features)*length(OctShifts)*length(Attens_dB)*length(HearingAids));
    Computed_UpdateRate_Hz_List=zeros(1,length(Features)*length(OctShifts)*length(Attens_dB)*length(HearingAids));
    Used_UpdateRate_Hz_List=zeros(1,length(Features)*length(OctShifts)*length(Attens_dB)*length(HearingAids));
    Used_FeatureTarget_Hz_List=zeros(1,length(Features)*length(OctShifts)*length(Attens_dB)*length(HearingAids));
    Features_List=cell(1,length(Features)*length(OctShifts)*length(Attens_dB)*length(HearingAids));
    OctShifts_List=zeros(1,length(Features)*length(OctShifts)*length(Attens_dB)*length(HearingAids));
    Levels_dBSPL_List=zeros(1,length(Features)*length(OctShifts)*length(Attens_dB)*length(HearingAids));
    NoiseAttens_dB_List=zeros(1,length(Features)*length(OctShifts)*length(Attens_dB)*length(HearingAids));
    Llist_List=cell(1,length(Features)*length(OctShifts)*length(Attens_dB)*length(HearingAids));
    Rlist_List=cell(1,length(Features)*length(OctShifts)*length(Attens_dB)*length(HearingAids));
    dBreTONE_List=zeros(1,length(Features)*length(OctShifts)*length(Attens_dB)*length(HearingAids))+dBreTONE; 
    HearingAid_List=zeros(1,length(Features)*length(OctShifts)*length(Attens_dB)*length(HearingAids));
    
    condIND=0;
    for FeatIND=1:length(Features)
        for shiftIND=1:length(OctShifts)
            for levelIND=1:length(Attens_dB)
                for aidIND=1:length(HearingAids)
                    condIND=condIND+1;
                    ...
                    HearingAid_List(condIND)=HearingAids(aidIND);
%                     if (Levels_dBSPL_List(condIND)~=120) && (HearingAid_List(condIND)==2)
%                         % Use nonlinear noise prescription
%                         HearingAid_List(condIND)=3;
%                     end
    ...
    Inloop.Name                         = 'DALinloop_NI_SCCi2_wavfiles_aid';
    if strcmp(stimulus_vals.Inloop.use_TONE,'yes')
    ...
    Inloop.params.Used.HearingAid_List             = HearingAid_List;
    ...
    if isempty(max_dBSPL)
        ...
        NoiseAttens_dB_List(3:3:end)=max_dBSPL(1)-Levels_dBSPL_List(1)+dBreTONE_noise; %adjust noise atten #3 to 0dB SNR
    ...
    DAL.short_description   = 'EHvLTASSrBFi';
